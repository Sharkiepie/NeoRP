...
local function MakeTeamInfo( team, TeamIcon )
	...
	nextModel.DoClick = function()
		...
		print("Next model: "..TeamIcon.team_models[ ModelPreview.model_id ])
...
local function CreateJobPanel( cteam, dlist )
	...
	icon:SetTitle( cteam.name )
	
	icon.ChangeTeam = function()
		LocalPlayer():ConCommand("say /"..cteam.command.." "..tostring( icon.model_id ) )
		NRP.mainmenu:Close()
	end
